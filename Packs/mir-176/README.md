...
## 小地图：漏在 mapinfo 之外的一整个文件

`MapInfo.MiniMap` 默认是 0，而浏览器和原版客户端都按这个号去 `mmap` 库取图，所以在这之前
**整个包 386 张地图，雷达和大地图全是空的**。号码不在 `mapinfo.txt` 里，在
`Envir/MiniMap.txt`——一个单独的文件，165 行 `地图文件 图号`，这就是它被整体漏掉的原因。

这批号码**不需要重编**，和怪物贴图正好相反。用 Crystal 自带的 classic 数据库当独立对照：
165 条里 115 条与 classic 给同名地图的值完全一致，1 条不一致（`6`，1.76 根本没声明这张图），
其余 49 条 classic 自己没设。八个主城八个对上——比奇省 101、沃玛森林 102、白日门 103、
丛林迷宫 104、毒蛇山谷 105、盟重省 106、封魔谷 121、苍月岛 161。这个编号是 WeMade 自己的，
两边服务端都照抄。再把 101 号解出来看了一眼：带护城河的方城、南北向的河、两侧森林——正是比奇省。

现在 386 张里 146 张有小地图（165 条里 19 条点的是 1.76 没声明的地图）。剩下 240 张是店铺和
室内，M2 本来也没给它们小地图。`BigMap` 跟着取同一个号：1.76 只有一列，而 classic 的 463
张里有 396 张这两个字段本来就相等。

### 但地图本身和 classic 不是一张

这是用户指出来的，查了确实如此。1.76 的比奇省是 700×700 的 OldSchool 格式，classic 那张
`0.map` 是另一种格式、12.7 MB，两个文件的哈希不同——**不是同一张图**。苍月岛同理：包里是
800×800，classic 的 `5.map` 完全是另一个文件。1.76 的比奇省有打重装衣服的荷花池，classic
那张没有。

所以两件事要记住：服务端读的是**包里的地图**（`contentRoot` 指向 `Packs/mir-176`），网关也
必须用 `--maps` 指向包里那份，见下面那一节；而小地图美术是 WeMade 的原图，在几何不同的地图上
雷达的底图和脚下的地形会有偏差。这一条只有进游戏才能判，我没有登录。

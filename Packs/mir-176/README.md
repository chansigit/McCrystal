...
但同一个 `Appr` 必然是同一个外观。所以拿已经配对好的怪反推出一张 `Appr -> Image` 对照表，
就能把没配上的怪补出来，依据写成 `appr:<同外观的那只怪>`，一眼能查。

这样补上了 39 只。有些看着"不像"，其实是对的——血僵尸和血巨人在 1.76 里共用 `Appr` 115，
本来就长一个样；蜜蜂和蝙蝠共用 42。要忠实还原 1.76，就该照它自己的复用来。

对照表并非处处唯一：99 个 `Appr` 里有 13 个对应到多个 `Image`，因为 Crystal 给了 1.76
只用一张图的怪各自的图（蝎子/蝎子王/圣域魔蝎在 1.76 里同为 `Appr` 83）。这些不自动选，
留给人工。

还差 39 只，列在 `monster-sprites-todo.txt`：8 只有候选待二选一，31 只（后期 BOSS 居多）
没有任何线索。同一份文件末尾附了 Crystal 尚未被本包认领的 195 种精灵图供挑选。
